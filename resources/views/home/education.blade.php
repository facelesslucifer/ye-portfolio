<div class="row" id="education" data-sr>
    <div class="col s12">
        <div class="card indigo-lighten-5 z-depth-1">
            <div class="card-content">
                <span class="card-title"><h4>Education</h4></span>
                <div class="card-row">
                    <div class="card-company">
                        National University of Singapore, ISS
                    </div>
                    <div class="contect-list">
                        <i class="fa fa-graduation-cap prefix material-icons-cust"></i>
                        <div>
                            <div class="cust-font">Systems Analysis</div>2011 - 2012
                        </div>
                    </div>
                    <div class="contect-list">
                        <i class="material-icons prefix">location_on</i>
                        <div>
                            Singapore
                        </div>
                    </div>
                    <div class="contect-list">
                        <i class="material-icons prefix">web</i>
                        <div>
                            <a href="https://www.iss.nus.edu.sg/" target="_blank">
                                iss.nus.edu.sg
                            </a>
                        </div>
                    </div>
                    <div class="contect-list">
                        <i class="material-icons prefix">book</i>
                        <div>
                            Graduate Diploma in Systems Analysis
                        </div>
                    </div>
                </div>
                <div class="divider"></div>
                <div class="card-row">
                    <div class="card-company">
                        University of Computer Studies, Yangon
                    </div>
                    <div class="contect-list">
                        <i class="fa fa-graduation-cap prefix material-icons-cust"></i>
                        <div>
                            <div class="cust-font">Computer Science</div>2003 - 2007
                        </div>
                    </div>
                    <div class="contect-list">
                        <i class="material-icons prefix">location_on</i>
                        <div>
                            Myanmar, Yangon
                        </div>
                    </div>
                    <div class="contect-list">
                        <i class="material-icons prefix">web</i>
                        <div>
                            <a href="http://www.ucsy.edu.mm/ucsy/" target="_blank">
                                www.ucsy.edu.mm
                            </a>
                        </div>
                    </div>
                    <div class="contect-list">
                        <i class="material-icons prefix">book</i>
                        <div>
                            Bachelor of Computer Science (Honours)
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>